...
// Class performs filtration to identify and validate token of incoming request. It will allow/block requests when token is validated.
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
        ...
        if(token != null) {
            // parse token
            String customer = JWT.require(Algorithm.HMAC512(AuthenticationConfigConstants.SECRET.getBytes()))
                    .build()
